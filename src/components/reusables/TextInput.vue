...
<template>
    <div>
        <label class="block mt-4 mb-2 text-xs tracking-wide font-medium uppercase  text-zinc-600 dark:text-neutral-200" :for="name">
            <slot></slot>
        </label>
        <div class="relative">
            <input :id="name" :type="inputType" :placeholder="placeholder" v-model="value" v-bind="$attrs" :class="[
                'block w-full rounded-md bg-white dark:bg-zinc-700 px-3  md:text-base text-zinc-800 dark:text-neutral-200 outline-1 -outline-offset-1 placeholder:text-zinc-400 focus:outline-2 focus:-outline-offset-2 text-sm/6',
