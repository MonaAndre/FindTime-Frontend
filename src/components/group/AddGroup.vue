    ...
    <div>
        ...
        <div v-if="showForm">
            <form class="form" @submit.prevent="createGroup">
                <TextInput :placeholder="'Group name'" :type="'text'" :name="'group-name'"
                    v-model="createGroupForm.groupName">Group name</TextInput>
                ...
                <div v-for="(email, index) in newMembersEmails" :key="index" class="flex gap-2 items-center">
        ...
        <div v-if="groupResponse?.failedEmails.length! > 0">
            ...
            <ul v-for="failedEmail in groupResponse?.failedEmails" :key="failedEmail">
                <li>{{ failedEmail }}</li>
            </ul>
        </div>
