...
<template>
    <section>
        <h3 class="text-center font-bold text-2xl my-5">Manage group categories for events</h3>
        <AddCategory @update="handleGetCategories(props.groupId)" :group-id="props.groupId" />
        <h4>category-list</h4>
        <ul class="flex gap-10 my-5" v-if="categoryList.length > 0">
            <li v-for="category in categoryList" :key="category.categoryId" class="flex  gap-3 items-center">
